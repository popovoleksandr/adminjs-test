...
function copyPackageJson() {
    ...
    gulp.src('build/app.js.map', {read: false}).pipe(gulpClean());
    return gulp.src('package.json').pipe(gulp.dest('build/'));
}

function copyDbConfigDirectory() {
    console.log("Copying config/config.json structure to build directory.");
    return gulp.src(['config/*'], {base:"."}).pipe(gulp.dest('build/config/'));
}
...
exports.build = build;
exports.clean = clean;
exports.copyDbConfigDirectory = copyDbConfigDirectory;
exports.default = gulp.series(clean, build,copyDbConfigDirectory);